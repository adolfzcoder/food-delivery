...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST["submit"])) {
    $stmt = $db->prepare("INSERT INTO orders (user_order, total, quantity,name, location) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss",$user_order, $totalPrice, $totalQuantity, $name, $location);

    if ($stmt->execute()) {
        echo "Submitted successfully!";
    } else {
        echo "Error inserting data:, if error persists, Call Adolf " . $stmt->error;
    }
    $stmt->close();
  }
}
